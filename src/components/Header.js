...
function Header() {
  ...
  return (
  <header className={'fixed top-0 z-10 w-full backdrop-blur bg-gray-900 bg-opacity-0 flex-column  justify-between py-4 md:py-8 px-6 md:px-10 duration-150'}>
      <div className={'flex items-center justify-between px-4 py-2 sm:p-0 relativ'}>
        ...
        {/* Desktop nav */}
      <div className={'hidden md:flex md:flex-row md:float-right space-x-6 md:space-x-8 duration-150'}>
      ...
      {/* Mobile nav */}
      {isMenuOpen && (
        <div className={'flex items-center md:float-right space-x-6 md:space-x-8 duration-150'}>
        <Link to={`/work`} className={defaultStyle} activeClassName={activeStyle} partiallyActive={true}>
            Work
        </Link>
        <Link to={`/writing`} className={defaultStyle} activeClassName={activeStyle} partiallyActive={true}>
