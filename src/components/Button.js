...
function Button({ primary, link, target, text, textColor }) {
  ...
  if (primary){
    return (
      <a href={link} target={target} className={`bg-white dark:bg-${textColor}-100 rounded-lg px-4 py-3 tracking-tighter font-semibold text-center text-${textColor} hover:text-white hover:bg-black hover:bg-opacity-60 duration-100`}>
        {/* purgecss: text-slate-900 text-yellow-700 text-sky-700 text-purple-800 bg-purple-800 bg-blue-700 bg-green-800 */}
        {text}
      </a>
    )
  } else {
